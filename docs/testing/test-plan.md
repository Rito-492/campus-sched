...
### 4.1 覆盖判定规则（04-协作规程 §3：测试经理决定哪些测试已实际覆盖）

每轮集成/里程碑结束时，由测试经理按 test-design §0 映射表逐条判定，规则如下：

- **已覆盖**：用例真实执行过，且有可查证据（测试输出/日志/截图路径写入 handoff）。执行记录须含真实 commit/构建版本。
- **部分覆盖**：仅子步骤执行，或断言少于用例设计的观察点（例如只断言 HTTP 码未查库）。
- **未执行**：代码存在但没跑过；计划中写了但没实现。**均不得计入覆盖率分子**。
- **不算覆盖的三种情形**（红线）：
  1. 用 Mock/替身冒充联调结果——M1 允许单模块测试 Mock 端口，M2/M3 不允许用 Mock 服务冒充联调完成（04 §3）；
  2. 测试代码存在但因阻塞从未运行（记录为阻塞，责任归属写明）；
  3. 以"构建成功/页面能打开"代替业务断言通过。
- 判定结论写入 `test-design.md` §0 状态列与 `test-report.md`；与实现方有分歧时以是否有真实执行证据裁决，必要时提请 PM 协调。
